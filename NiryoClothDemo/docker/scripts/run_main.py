...
class ClothDemo:
    ...
    def predict(self, image):
        """
        Predicts the cloth corner positions and angle from the input image.
        
        Args:
            image (numpy.ndarray): The input image.
        
        Returns:
            image (numpy.ndarray): The input image with detected corners drawn on it.
            chosen_X_Y_angle (list): A list of tuples containing the x, y coordinates and angle of each detected corner.
        """
        ...
        with torch.inference_mode():
            output_batch_ = self.model(model_input).detach()
            center_output = self.center_model(output_batch_)

        center_pred = center_output["center_pred"].detach()
        pred_angle = center_output["pred_angle"].detach()
        ...
        for prediction, angle in zip(predictions, angles):
            ...
            list_angle = np.array2string(angle, precision=6, suppress_small=True)
            
            chosen_X_Y_angle.append((x1, y1, list_angle.strip("[]")))
        # cv2.namedWindow("Output", cv2.WINDOW_NORMAL)
        # cv2.resizeWindow("Output", 1280,720)
        # cv2.imshow("Output", image)
        # cv2.waitKey(1)
        torch.cuda.empty_cache()
        return cv2.cvtColor(image, cv2.COLOR_BGR2RGB), chosen_X_Y_angle
