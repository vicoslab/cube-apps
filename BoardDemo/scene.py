...
from gui_components import TextFieldMultilingual, Language

import time

i8n_button_detect = {
    Language.EN: "Recognise",
    Language.SL: "Razpoznaj"
}
def get_scene(parameters):
    ...
    parameters.state.board_detection_time    = None
    parameters.state.board_waiting_detection = False
    ...
    def toggle_detection(button: Button, gui: Gui, state):
        ...
        if echolib_handler.docker_channel_out is not None:
            ...
            parameters.state.board_waiting_detection = True
